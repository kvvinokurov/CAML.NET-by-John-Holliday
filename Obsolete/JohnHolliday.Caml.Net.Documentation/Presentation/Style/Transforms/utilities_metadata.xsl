...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.1"
  ...
  <xsl:template name="insertMetadata">
      ...
      <!-- Abstract -->
      <xsl:choose>
        <xsl:when test="string-length($summary) &gt; 254">
          <MSHelp:Attr Name="Abstract" Value="{concat(substring($summary,1,250), ' ...')}" />
        </xsl:when>
        <xsl:when test="string-length($summary) &gt; 0">
          <MSHelp:Attr Name="Abstract" Value="{$summary}" />
        </xsl:when>
      </xsl:choose>
...
<xsl:template name="helpPriorityMetadata">
  ...
  <xsl:template name="GetDevLangAttrValue">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$devlang = 'VisualBasic' or $devlang = 'vb' or $devlang = 'vb#' or $devlang = 'VB' or $devlang = 'kbLangVB'" >
